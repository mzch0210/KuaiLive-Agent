...
Let $a(Z)\in\{0,1\}$ denote a serving-time policy, where $a(Z)=0$ uses the base ranking and $a(Z)=1$ uses the specialist ranking. The expected improvement over always using the base satisfies
\begin{align}
...
\begin{equation}
    \begin{aligned}
        ...
        \text{s.t.}
        &\quad \sum_{i=1}^{n}a_i=m,
    \end{aligned}
    \qquad
    a_i=\mathbf{1}[i\in\mathcal{I}_m],\quad
    \mathcal{I}_m=\operatorname{TopIdx}_{m}\!\left(\hat{\eta}_r(z_1),\ldots,\hat{\eta}_r(z_n)\right),
    \label{eq:budget-decision}
\end{equation}
with ties resolved by a fixed deterministic rule. Holding $m$ constant across competing selectors fixes the invocation rate, so matched-budget comparisons isolate differences in event selection rather than differences in specialist usage. Replacing $\hat{\eta}_r(z_i)$ with the realized offline $\Delta_m(x_i)$ yields the exact-budget Oracle used only as an analysis upper bound; it is unavailable to the serving policy.
...
\begin{equation}
    ...
    \label{eq:cost-rule}
\end{equation}
We use Eq.~\eqref{eq:cost-rule} only as a deployment interpretation of selective specialist use; the controlled experiments avoid choosing $\lambda$ and instead compare policies at the same exact invocation budget. General deferral and routing rules are established in the literature discussed in Section~2.
