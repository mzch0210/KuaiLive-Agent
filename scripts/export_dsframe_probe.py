...
def _history_str(seq) -> str:
    """Serialize history exactly as DS-Frame BaseReader/eval_list_columns expects."""
...
def main():
    ...
    for uid,g in tr.groupby('user_id',sort=False):
        ...
        for j in range(1,len(seq)):
            ...
            if rng.random() <= a.train_fraction:
                hist=seq[max(0,j-a.history_max):j]
                train_rows.append({'user_id':u,'item_id':seq[j],'history_id':_history_str(hist)})
    ...
    for uid in sorted(eval_users):
        ...
        valid.append({'user_id':u,'item_id':d,'history_id':_history_str(h)})
        test.append({'user_id':u,'item_id':t,'history_id':_history_str((h+[d])[-a.history_max:])})
    out=a.out_root/a.dataset; out.mkdir(parents=True,exist_ok=True)
    cols=['user_id','item_id','history_id']
    pd.DataFrame(train_rows,columns=cols).to_csv(out/f'{a.dataset}.train.remap.csv',index=False)
    ...
    meta={'dataset':a.dataset,'train_fraction':a.train_fraction,'probe_train_rows':len(train_rows),'estimated_full_train_rows':total_full,'eval_users':len(eval_users),'item_universe':len(imap),'history_max':a.history_max,'seed':a.seed,'schema':cols,'history_serialization':'space-separated integer ids'}
