...
def main():
    ...
    ap.add_argument(
        '--require-active-at-target',
        action=argparse.BooleanOptionalAction,
        default=True,
        help='If true, a prior unclicked exposure must still be live at the target click time. '
             'If false, evaluate a recent pre-click exposure episode instead of a contemporaneous slate.',
    )
    args = ap.parse_args()
    ...
    room_map[['item_id', 'live_id', 'streamer_id']] = room_map[
        ['item_id', 'live_id', 'streamer_id']
    ].astype('int64')
    ...
    rr[['live_id', 'start_timestamp', 'end_timestamp']] = rr[
        ['live_id', 'start_timestamp', 'end_timestamp']
    ].astype('int64')
    lifecycle = {
        int(r.live_id): (int(r.start_timestamp), int(r.end_timestamp))
        for r in rr.itertuples(index=False)
    }
    lookback_ms = int(args.lookback_hours * 3600 * 1000)

    protocol = (
        'official_negative_strict_active'
        if args.require_active_at_target
        else 'official_negative_recent_preclick_episode'
    )
    definition = (
        'negative.csv exposures shown-but-not-clicked; prior configurable lookback and still active at target'
        if args.require_active_at_target
        else 'negative.csv exposures shown-but-not-clicked; most recent distinct rooms strictly before target within configurable lookback; no target-time activity requirement'
    )
    ...
    def build_pool(targets):
        ...
        for r in targets.itertuples(index=False):
            ...
            vals = []
            if g is not None:
                # Never use future exposure. When active-at-target is disabled,
                # this becomes an episode-level choice set: previously shown-but-skipped
                # rooms versus the later clicked target, not a contemporaneous slate.
                ...
                for q in z.iloc[::-1].itertuples(index=False):
                    ...
                    if args.require_active_at_target:
                        ...
                        if life is None or not (life[0] <= t < life[1]):
                            continue
                    vals.append(live)
                    seen.add(live)
            pools[uid] = vals
    ...
    dp, dc = build_pool(dev)
    tp, tc = build_pool(test)
    eligible = [u for u in users if len(dp.get(u, [])) >= args.k and len(tp.get(u, [])) >= args.k]
    elig = set(eligible)
    if len(eligible) < args.min_users:
        # Still write a coverage report before stopping so protocol support is auditable.
        ...
        meta = {
            ...
            'require_active_at_target': args.require_active_at_target,
    ...
    dv, dev_raw = base_rows(dev)
    te, test_raw = base_rows(test)

    def neg_items(x, pools):
        return [str([live2item[v] for v in pools[int(u)][:args.k]]) for u in x.user_id.astype(int)]

    dv['neg_items'] = neg_items(dev_raw, dp)
    te['neg_items'] = neg_items(test_raw, tp)

    out = args.out_root / args.dataset
    out.mkdir(parents=True, exist_ok=True)
    # Keep the full frozen training set; dev/test are the exposure-covered cohort.
    ...
    for r in test_raw.itertuples(index=False):
        ...
        # Fair test history includes the dev target as observed before test.
        drow = dev[dev.user_id.astype(int) == int(r.user_id)]
        if len(drow):
            hist = set(hist) | set(drow.streamer_id.astype(int))
    ...
    meta = {
        ...
        'definition': definition,
        'interpretation': (
            'contemporaneous target-time candidate ranking'
            if args.require_active_at_target
            else 'recent-exposure episode ranking; negatives were genuinely exposed earlier and skipped, but need not be live at target click time'
        ),
        'k': args.k,
        'candidate_count': args.k + 1,
        'lookback_hours': args.lookback_hours,
        'require_active_at_target': args.require_active_at_target,
        'future_exposures_used': False,
        'all_users': len(users),
    ...
    (out / 'export_meta.json').write_text(json.dumps(meta, indent=2) + '\n')
    pd.DataFrame({
        'user_id': te.user_id.astype(int),
        'familiar_negative_count': fam,
    }).to_csv(out / 'test_candidate_diagnostics.csv', index=False)
    print(json.dumps(meta, indent=2))
