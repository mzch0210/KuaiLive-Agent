...
def bootstrap_delta(diff, seed=SEED, n_boot=5000):
...
def load_data(root: Path, n_neg: int):
    ...
    return train, dev, test, item_to_streamer


def streamer_sequence(df, item_to_streamer):
    items = df.item_id.astype(int).to_numpy()
    return [int(item_to_streamer[int(i)]) for i in items]


def popularity_by_streamer(train, item_to_streamer):
    streamers = pd.Series(streamer_sequence(train, item_to_streamer), index=train.index)
    p = np.log1p(streamers.value_counts().astype(float))
    if len(p):
        p = (p - p.min()) / max(float(p.max() - p.min()), EPS)
    return {int(k): float(v) for k, v in p.items()}


def histories(df, item_to_streamer, short_k=10):
    out = {}
    for uid, g in df.groupby("user_id", sort=False):
        sids = streamer_sequence(g, item_to_streamer)
        c = Counter(sids)
        maxc = max(c.values()) if c else 1
        long = {i: n / maxc for i, n in c.items()}
        short = {}
        for dist, sid in enumerate(reversed(sids[-short_k:])):
            short[sid] = max(short.get(sid, 0.0), math.exp(-dist / 3.0))
        out[int(uid)] = (short, long)
    return out


def memory_scores(cands, uid, pop, hist, item_to_streamer):
    short, long = hist.get(uid, ({}, {}))
    sids = [int(item_to_streamer[int(i)]) for i in cands]
    p = np.fromiter((pop.get(sid, 0.0) for sid in sids), float, count=len(sids))
    s = np.fromiter((short.get(sid, 0.0) for sid in sids), float, count=len(sids))
    l = np.fromiter((long.get(sid, 0.0) for sid in sids), float, count=len(sids))
...
def score_memory(phase, history_df, pop, n_neg, item_to_streamer):
    hist = histories(history_df, item_to_streamer)
    rows = []
    for r in phase.itertuples(index=False):
        ...
        rank = target_rank(cands, memory_scores(cands, int(r.user_id), pop, hist, item_to_streamer))
...
def user_features(history_df, item_to_streamer):
    rows = []
    for uid, g in history_df.groupby("user_id", sort=False):
        streamers = streamer_sequence(g, item_to_streamer)
        times = g.time.astype("int64").tolist(); n = len(streamers)
        recent_n = min(10, max(2, n // 3)) if n >= 3 else max(1, n // 2)
        old = streamers[:-recent_n] if n > recent_n else streamers[:max(1, n // 2)]
        recent = streamers[-recent_n:] if recent_n else streamers
        uniq = len(set(streamers))
        rows.append({
            "user_id": int(uid), "history_len": n,
            "repeat_rate": 1 - uniq / max(n, 1),
            "preference_entropy": norm_entropy(streamers),
...
def hgb():
    return HistGradientBoostingRegressor(
        max_iter=200, learning_rate=.05, max_depth=3,
...
def main():
    ...
    train, dev, test, item_to_streamer = load_data(args.data_dir, args.n_neg)
    ...
    report = {
        "task": "next_live_room",
        "memory_expert": "frozen streamer relationship MemoryFusion projected to active room",
        "users": int(len(t)),
        ...
        "kill_gate_pass": bool(delta > 0 and lo > 0),
        "kill_gate_rule": "PASS iff SelectiveMemory > SASRec and paired bootstrap 95% CI lower bound > 0",
        "protocol": "Frozen v2 HGB; streamer-state features; SASRec confidence; 5-fold OOF dev threshold; test evaluation only.",
    }
